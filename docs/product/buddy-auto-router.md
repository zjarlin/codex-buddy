...
点击 Codex 模型 chip 时，先关闭当前 Host 的 Auto Router 接管和隐私改写，再调用当前原生菜单的模型选择回调。原生支持的思考强度会保留，不支持时沿用原生菜单的模型默认值。配置失败会显示错误并保留原模型；会话或 Host 切换后不对旧目标继续应用选择。配置完成前禁止发送，防止消息抢先使用旧模型。
...
配置面板默认折叠，展开后按“路由”“中断会话”“任务详情”分 tab；路由配置使用双栏紧凑布局，帮助说明放在对应控件提示中；中断恢复入口同时位于中断 tab 和侧边栏会话状态图标。
...
侧边栏标题左侧的状态图标可直接恢复中断会话；展开 Auto Router 后也会列出“最近中断会话”，显示标题和终态，点击“恢复”可手动调取。侧边栏图标鼠标悬停提示“会话已中断，点击恢复”，支持 Enter / Space；点击不会触发父行导航。两个入口恢复期间都会禁用按钮，成功后移除列表项或还原原生状态槽，失败显示原因并允许再次点击。

分页扫描未归档 Codex 原生会话，标记最新回合明确为 failed、interrupted 或 cancelled 的记录，包含用户手动停止。按侧边栏 Host 隔离读取持久历史，每 15 秒最多检查一次，窗口重新聚焦时刷新；无需打开配置面板，重启后仍能发现记录，不另存聊天内容。运行中的会话使用官方 `thread.status.type === "active"` 显示独立的高亮状态，不显示恢复按钮；外部 Harness、隐私模式或关闭自动路由时不提供恢复入口。单条历史读取失败不阻断其他会话；整轮发现失败保留上一次成功状态，不把选中行当作运行状态。
...
会话行的三点菜单提供“归档项目内已完成会话”。它以当前会话的原生 `cwd` 为项目边界，只归档同项目、未归档、最新回合为 completed / succeeded 且未在运行的 Codex 原生会话；返回归档、跳过和失败数量。官方协议没有独立的项目行菜单入口，因此操作放在项目内任意会话行上，表示作用范围而不是只处理当前会话。
