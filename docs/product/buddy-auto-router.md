...
开启“JEV 判断”（默认开启）时，普通在线回合在评级阶段调用一次 JEV System One，批量判断路由入口、复杂度档位、是否具破坏性、当前请求是否要求推送，以及执行角色。JEV 返回的难度、意图和角色由 Host 采用；逐题依据（`route`、`complexity`、`destructive`、`push`、`role`）以 `judgment` 写入本任务状态并在面板“JEV 判断”一行显示，格式为 `模型 · 逐题状态`。
...
“JEV 判断”下方提供 API Key 与网关地址两个输入框。密钥写入 `CODEX_HOME/buddy-jev.json`（权限 `0600`，独立于设置文件），优先级高于 `TYPESAFE_API_KEY` / `TYPESAFE_BASE_URL` 环境变量；Host 每回合重新加载。网关地址留空表示使用官方 `https://api.typesafe.ai`，也可填自建 Sub2API 网关（需转发 `/v1/systemone`）。快照只回传“已配置/未配置”和网关地址，密钥输入框为密码类型且保存后立即清空，密钥不回传浏览器、不写入 `settings`。清除按钮同时清空密钥与网关地址并停止 JEV 判断。

判定优先级：用户显式指定的执行角色最高；未指定时 JEV 判定优先，缺少 JEV 或调用失败时退回本地 `specialist` 与 `assessWithContext` 规则。推送模型旁路以 JEV 的推送判断为准；缺少 JEV 时退回本地正则预筛，正则只决定“是否值得询问 JEV”。JEV 超时、认证、限流、网络或协议错误都会被捕获并记录，随后用本地规则完成该回合，不阻断原生请求。
...
- JEV 判断：JEV 模型名和逐题状态（automatic / review / defer）。这是 System One 判断结果，不是 LLM 推理，也不授予执行权限。
