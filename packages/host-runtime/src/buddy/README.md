...
`judgment.ts` 是 System One（JEV 或本地 Laya）的唯一接入点，也是意图识别、工具路由与执行角色判断的唯一来源：普通在线回合批量判断路由、复杂度、破坏性、推送意图、执行角色、普通问答、承接上文与精确 CLI 入口，失败时回退本地兜底；密钥只从 Host 环境读取，模型名按网关平台选择。`git-push-bypass.ts` 为旁路回合附带原生 Git/平台技能并统计本次 Host 运行期间每个模型的旁路回合完成率；它只在 System One 确认推送后生效，不调用夯、不扩大权限、不把回合完成率当作 Git 推送验收，也不自动重放失败写操作。
