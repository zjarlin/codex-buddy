...
收藏栏旁和收藏菜单搜索框旁均提供“刷新模型”。Codex 使用当前 Host 的原生模型查询重新读取目录，外部 Harness 使用其 `inspectHarness(refresh: true)` 接口；刷新不修改当前模型、思考强度、收藏或自动规划设置。刷新时禁止重复操作，保留已打开菜单和搜索内容；失败显示原因并允许重试，切换 Host 或会话后不显示旧请求的错误。隐私模式拒绝在线目录刷新。
...
此入口刷新客户端目录，不安装后台同步任务，也不改写供应商配置。自定义供应商新增模型需先由现有 Buddy 同步器更新 `model_catalog_json`（可运行 `npx -y codex-buddy sync`）；如果运行中的 App Server 尚未重新加载该文件，仍需要重启客户端。原生刷新绑定不可用时明确提示，不用路由候选伪装成原生可选模型。
